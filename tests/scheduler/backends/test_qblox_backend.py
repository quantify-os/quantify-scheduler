...
def test_expand_from_normalised_range():
    ...
    expanded_val = QASMProgram.expand_from_normalised_range(
        1, constants.MAX_WAIT_TIME_IMMEDIATE, "test_param", acq
    )
    assert expanded_val == constants.MAX_WAIT_TIME_IMMEDIATE // 2
    with pytest.raises(ValueError):
        QASMProgram.expand_from_normalised_range(
            10, constants.MAX_WAIT_TIME_IMMEDIATE, "test_param", acq
        )
