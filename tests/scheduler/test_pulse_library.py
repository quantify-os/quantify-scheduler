...
from quantify.scheduler.resources import ClockResource, PortResource

cl01 = ClockResource('cl01', freq=6.81e9)
cl01 = ClockResource('cl01', freq=6.81e9)
p01 = PortResource('p01')


def test_operation_duration_single_pulse():
    dgp = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3,
                    duration=20e-9, clock=cl01, port=p01)
...
def test_operation_duration_composite_pulse():
    dgp1 = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3,
                     duration=10e-9, clock=cl01, port=p01, t0=0)
...
def test_pulses_valid():
    sqp = SquarePulse(amp=.5, duration=300e-9, port=p01)
    msqp = ModSquarePulse(amp=.5, duration=300e-9, clock=cl01, port=p01)
    dgp = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3,
                    duration=20e-9, clock=cl01, port=p01)
