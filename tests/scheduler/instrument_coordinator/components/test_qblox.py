...
def test_looped_acquisition(close_all_instruments, make_qrm):
    class LoopedPeriodicAcquisition(acquisition_library.SSBIntegrationComplex):
        def __init__(
            self,
            port: str,
            clock: str,
    ...
    compiled_sched = qcompile(sched, DEVICE_CFG, HARDWARE_MAPPING)
    qrm: qblox.PulsarQRMComponent = make_qrm("qrm0", "1234")
    qrm.prepare(compiled_sched.compiled_instructions['qrm0'])
    qrm.start()
    qrm.retrieve_acquisition()
